...
import { getOTPFromRedis, deleteOTPFromRedis } from "@/lib/redis";
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      storedOtp = await getOTPFromRedis(userId);
      if (storedOtp) {
        isFromRedis = true;
    ...
    // Clear from Redis
    try {
      await deleteOTPFromRedis(userId);
